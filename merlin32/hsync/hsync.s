...
:go
		; if we get stable numbers here, we are in sync
		lda RAST_COL
		ldx RAST_COL+1
...
;------------------------------------------------------------------------------
;
; lda # of clocks to burn
;
; Note function will always burn a fixed amount of cycles + the # you passed in
;
slip_clocks
		and #$F  ; right no burn 0->4
		asl
		tax
		jmp (:bloop,x)
:bloop
